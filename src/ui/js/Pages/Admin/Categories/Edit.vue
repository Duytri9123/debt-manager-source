...
const existingImage = props.category.img_url
  ? (props.category.img_url.startsWith('http') ? props.category.img_url : `/storage/${props.category.img_url}`)
  : null
    ...
    <div class="space-y-6">
      ...
      <form @submit.prevent="submit" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
        ...
        <div>
          ...
          <!-- Existing image -->
          <div v-if="existingImage && !imagePreview" class="mb-3">
            <p class="text-xs text-gray-500 mb-1">Ảnh hiện tại:</p>
            <img :src="existingImage" alt="Current" class="h-24 w-24 rounded-lg object-cover border border-gray-200" />
          </div>

          <!-- New image preview -->
          <div v-if="imagePreview" class="relative mb-3 inline-block">
            <img :src="imagePreview" alt="Preview" class="h-24 w-24 rounded-lg object-cover border border-indigo-200" />
            <button type="button" @click="clearNewImage" class="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600">
              <X :size="10" />
            </button>
            <p class="mt-1 text-xs text-indigo-600">Ảnh mới (sẽ thay thế ảnh cũ)</p>
          </div>
          ...
          <label class="flex cursor-pointer items-center gap-2 rounded-lg border-2 border-dashed border-gray-300 px-4 py-3 text-sm text-gray-500 hover:border-indigo-400 hover:text-indigo-600 transition-colors">
            <Upload :size="16" />
            <span>{{ existingImage ? 'Thay ảnh mới' : 'Chọn ảnh' }} (jpg, jpeg, png, webp — tối đa 2MB)</span>
